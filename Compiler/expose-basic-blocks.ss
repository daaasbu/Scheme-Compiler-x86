...
(library (Compiler expose-basic-blocks)
	 ...
	 (define-pass expose-basic-blocks : LexposeMemoryOperands (x) -> LexposeBasicBlocks ()
	   (definitions
	   ...
	   (Tail : Tail (x) -> * (bb b*)
		 ...
		 [(alloc ,triv) (values (make-basic-block '() (in-context Tail `(alloc ,triv))) '())]
		 [(mref ,triv0 ,triv1) (values (make-basic-block '() (in-context Tail `(mref ,triv0 ,triv1))) '())])
	   ...
	   (Effect : Effect (ef after-bb) -> * (bb b*)
		   [(return-point ,l ,[* bb0 b1*]) (values bb0 (append (list (cons l (make-lambda after-bb))) b1*))]
		   [(nop) (values after-bb '())]
		   [(set! ,locrf0 ,[rhs1])
