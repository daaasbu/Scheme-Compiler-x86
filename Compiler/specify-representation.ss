...
;;MAY NEED TO ADD MORE SRA's IN MAKE LINES
	   (Value2 : Value (x) -> Value ()
		   [(,vprim ,[val*] ...) 
			...
			((make-procedure)   (let ([tmp (unique-name 'TMP)])
					     ; (printf "disp-proc: ~a \n cadr: ~a \n value: ~a \n" disp-procedure-data (cadr val*) (* disp-procedure-data (add1 (sra (cadr val*) shift-fixnum))))
					      `(let ([,tmp (prim + (alloc ,(* disp-procedure-data (add1 (sra (cadr val*) shift-fixnum)))) ,tag-procedure)])
						 (begin
